...
router.post("/", authenticate, authorize(["Admin", "Landlord"]), async (req, res) => {
  const {
    title,
    description,
    propertyType,
  ...
  if (!allowedCurrencies.includes(normalizedCurrency)) {
    return sendError(res, "Currency must be JOD or NIS", HTTP_STATUS.BAD_REQUEST);
  }

  if (!title || !description || !propertyType || !pricePerMonth || !bedrooms || !bathrooms || !squareFeet || !distanceToUniversity || !leaseDuration || !universityId) {
    return sendError(res, "Missing required fields", HTTP_STATUS.BAD_REQUEST);
  }

  if (Array.isArray(images) && images.length > 10) {
    return sendError(res, "Maximum 10 images per listing", HTTP_STATUS.BAD_REQUEST);
  }

  const transaction = await Listing.sequelize.transaction();

  try {
    const university = await require("../models").University.findByPk(universityId);
    if (!university) {
    ...
    if (Array.isArray(images) && images.length > 0) {
      ...
      if (payload.length) {
        createdImages = await ListingImage.bulkCreate(payload, { transaction });
      }
    }
    ...
    return sendError(res, "Failed to create property listing", HTTP_STATUS.SERVER_ERROR, error);
  }
});
